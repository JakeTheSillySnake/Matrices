...
START_TEST(s21_create_matrix_6) {
  int result = 0;
  matrix_t A = {0};
  result = s21_create_matrix(2, 4, &A);
  ck_assert_int_eq(result, OK);
  s21_remove_matrix(&A);
}
END_TEST
...
START_TEST(s21_mult_number_6) {
  ...
  result = s21_mult_number(&A, number, &B);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  ck_assert_int_eq(result, INCORRECT_MATRIX);
}
...
START_TEST(s21_calc_complements_3) {
  ...
  A.matrix[3][1] = 7.0;
  A.matrix[3][2] = 1.0;
  result = s21_calc_complements(&A, &B);
  s21_remove_matrix(&A);
  ck_assert_int_eq(result, CALCULATION_ERROR);
}
END_TEST
...
START_TEST(s21_determinant_1) {
  ...
  result = s21_determinant(&A, &determinant);
  ck_assert_int_eq(result, CALCULATION_ERROR);
  s21_remove_matrix(&A);
}
END_TEST
